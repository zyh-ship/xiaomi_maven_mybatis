...
<script type="text/javascript">
	function sendOrder(id){
	   $.post("/myxiaomi/adminorderservlet",{method:"sendOrder",oid:id},function (data) {
			$("#btn1"+id).hide();
        });
	/*confirm("确认发货吗？");
            location.href = "/myxiaomi/adminorderservlet?method=sendOrder&oid="+id;*/
	}
	$(function(){
		$("#search").click(function(){
			var username = $("input[name='username']").val();
			var status = $("select[name='orderStatus'] option:selected").val();
			location.href="${pageContext.request.contextPath}/adminorderservlet?method=search&username="+username+"&status="+status;
		})
	})
</script>
</head>
			...
			<div class="panel-body">
				...
				<table id="tb_list" class="table table-striped table-hover table-bordered table-condensed">
					...
					<tr>
						...
						<td>
							<c:if test="${order.status eq 2}">
								<button type="button" class="btn btn-danger btn-sm" id="btn1${order.id}" onclick="sendOrder('${order.id}')">发货</button>
							</c:if>
						</td>
					</tr>
					</c:forEach>
