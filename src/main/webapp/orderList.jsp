...
<script type="text/javascript">
	...
	function changeStatus(orderId){
	    if (confirm("确定收货吗?"))
	    $.post("${pageContext.request.contextPath}/orderservlet",{method:"changeStatus",oid:orderId},function (data) {
			if (data==1){
			    $("#"+orderId+"th").text("订单完成").css("color","red");
			    $("#"+orderId+"btn1").hide();
			}
        });
	}
			...
			<c:forEach items="${orderList}" var="order" varStatus="i">
				<tr>
					...
					<th>
						...
						<c:if test="${order.status eq 3 }">
							<button type="button" id="${order.id}btn1" class="btn btn-warning btn-sm" onclick="changeStatus('${order.id}')">确认收货</button>
						</c:if>
					</th>
				</tr>
			</c:forEach>
